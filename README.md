...
4. **三月十九破晓**：九根支撑柱清账。没有倒柱的撬点胜算为零且不掷骰；倒柱后胜算为 `10 + 权重`；人物的确定性行动也可能直接保证结果。随后落入七类正常结果之一，生成三层编年史——事实层（发生了什么）、记载层（什么被写下）、流传层（百年后怎么说）。全部种子骰在终局公开。
...
│   ├── storage.ts           # v5 单局存档（重放防篡改）+ v2 史鉴
│   └── *.test.ts            # 确定性/传播不变量/七类结局可达/防篡改/200 种子 fuzz
├── story/                   # 旧版三回合互动小说《刻下无名》（归档，无入口）
└── game/                    # 更早的十二回合策略引擎《煤山未尽》（归档，无入口）
```

存档键：`what-if-history.sim.v5`（单局）与 `what-if-history.codex.v2`（跨局史鉴）。旧版 `story.v3` 与 `autosave.v2` 存档原地保留，互不迁移。

实现细节见 [架构说明](docs/ARCHITECTURE.md)，史实与架空的分界见 [史实边界与资料](docs/HISTORICAL-NOTES.md)。

## 公开发布与可发现性

v0.3 的首页在 JavaScript 运行前也提供与实际玩法一致的静态正文，并补齐规范网址、分享卡片、站点地图和 `VideoGame + WebApplication` 结构化数据。元数据不虚构评分、奖项或玩家数量；搜索抓取与通用模型训练在 `robots.txt` 中分开声明。

```powershell
npm run audit:public
```

这条命令会核对元数据、结构化数据、静态正文、抓取边界、站点地图、版权声明和 1200×630 分享图。设计依据与维护清单见 [SEO/GEO 发布说明](docs/SEO-GEO.md)；原创形成、素材准入和“公开网页无法保证绝不被复制”的现实边界见 [原创性与权利说明](docs/ORIGINALITY-AND-RIGHTS.md)。
